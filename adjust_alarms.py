...
logger = logging.getLogger("adjust_alarms")
console = Console()
...
def add_adjustment(args):
    ...
    for adj in adjustments["adjustments"]:
        if adj["id"] == args.id:
            logger.error(f"Adjustment with ID {args.id} already exists. Use update instead.")
            return False
...
def remove_adjustment(args):
    ...
    adjustments["adjustments"] = [adj for adj in adjustments["adjustments"] if adj["id"] != args.id]
...
def main():
    ...
    # Update command
    update_parser = subparsers.add_parser("update", help="Update an existing manual adjustment")
