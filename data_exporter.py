        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for file_type in file_types:
                ...
                if os.path.exists(zip_path):
                    logger.info(
                        f"Skipping {file_type.upper()} for {period}: Output file {zip_path} already exists."
                    )
                    progress.update(
                        tasks[file_type],
                        completed=100,
                        description=f"Exporting {file_type.upper()} - skipped (exists)",
                    )
                    results[file_type] = (
                        True  # Mark as success since it's already done/skipped
                    )
                ...
                progress.update(
                    tasks[file_type],
                    completed=10,
                    description=f"Exporting {file_type.upper()} - started",
                )

                # Submit export task to executor
                futures[file_type] = executor.submit(
                    export_and_archive_tables, period, [file_type]
                )
            ...
            for file_type, future in futures.items():
                try:
                    ...
                    progress.update(
                        tasks[file_type],
                        completed=30,
                        description=f"Exporting {file_type.upper()} - querying DB",
                    )
                    ...
                    progress.update(
                        tasks[file_type],
                        completed=0,
                        description=f"Exporting {file_type.upper()} - failed",
                    )
...
def main(period):
    ...
    periods = (
        pd.date_range(start=period, end=period, freq="MS").strftime("%Y-%m").tolist()
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Export data from database to CSV files and create ZIP archives"
    )
