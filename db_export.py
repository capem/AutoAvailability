...
class DBExporter:
    ...
    def export_table_data(self, table_name, period, output_path):
        ...
        return self._export_regular_table(table_name, period_start, period_end, output_path)
    ...
    def _export_regular_table(self, table_name, period_start, period_end, output_path):
        ...
        try:
            ...
            with self.connection_pool.get_connection() as conn:
                ...
                if df.empty:
                    logger.warning(f"No data found for {table_name} in period {period_start} to {period_end}")
                    # Create an empty file with headers
                    df.to_csv(output_path, index=False)
                    return True
...
def export_and_archive_tables(period, file_types=None):
    ...
    try:
        for file_type in file_types:
            ...
            csv_path = os.path.join(export_dir, f"{period}-{file_type.lower()}.{FILE_EXTENSION}")
            zip_path = os.path.join(BASE_UPLOAD_PATH, file_type.upper(), f"{period}-{file_type.lower()}.zip")
