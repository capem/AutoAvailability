...
def main(period_range, period_start_dt, period_end_dt):
    ...
    results = pd.concat(
        [pd.read_pickle(f"./monthly_data/results/{dt.strftime('%Y-%m')}.pkl") for dt in period_range],
        ignore_index=True,
    )
    ...
    sum_columns = ["EL", "ELX", "ELNX", "EL_2006", "wtc_kWG1TotE_accum", "EL_PowerRed", "EL_Misassigned"]
    ...
    df_exploi.loc[title, "Indispo. SGRE énergie (%)"] = round(100 * (sums["ELNX_eq"]) / (sums["Epot_eq"]), 2)
    df_exploi.loc[title, "Indispo. SGRE énergie (MWh)"] = round(sums["ELNX_eq"] / 1e3, 2)
    df_exploi.loc[title, "Indispo. SGRE temps (%)"] = round(
        100 * results["Period 1(s)"].sum() / 3600 / 24 / 131 / days_in_period, 2
    )
    df_exploi.loc[title, "Indispo. SGRE temps (heures)"] = round(results["Period 1(s)"].sum() / 3600, 2)

    df_exploi.loc[title, "Indispo. TAREC énergie (%)"] = round(100 * (sums["ELX_eq"]) / (sums["Epot_eq"]), 2)
    df_exploi.loc[title, "Indispo. TAREC énergie (MWh)"] = round(sums["ELX_eq"] / 1e3, 2)
    df_exploi.loc[title, "Indispo. TAREC temps (%)"] = round(
        100 * results["Period 0(s)"].sum() / 3600 / 24 / 131 / days_in_period, 2
    )
    df_exploi.loc[title, "Indispo. TAREC temps (heures)"] = round(results["Period 0(s)"].sum() / 3600, 2)

    # Filter wind speed and direction columns that exist
    wind_speed_cols = [
        col
        for col in ["met_WindSpeedRot_mean_38", "met_WindSpeedRot_mean_39", "met_WindSpeedRot_mean_246"]
        if col in results.columns
    ]
    wind_direction_cols = [
        col
        for col in ["met_WinddirectionRot_mean_38", "met_WinddirectionRot_mean_39", "met_WinddirectionRot_mean_246"]
        if col in results.columns
    ]

    # Calculate mean wind speed if any columns are available
    if wind_speed_cols:
        df_exploi.loc[title, "Vent moyen Mâts (m/s)"] = round(results[wind_speed_cols].mean().mean(), 2)
...
def Top15(period_range, period_start_dt, period_end_dt):
    ...
    df_Top15.rename({"ELNX": "Energie perdue SGRE", "ELX": "Energie perdue TAREC"}, inplace=True, axis=1)
